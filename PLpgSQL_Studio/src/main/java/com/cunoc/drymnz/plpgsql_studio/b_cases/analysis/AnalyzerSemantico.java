...
import com.cunoc.drymnz.plpgsql_studio.a_entidades.analyzer.*;
...
public class AnalyzerSemantico {
    ...
    public AnalyzerSemantico() {
        this.listError = new ArrayList<ReportErrorInterpreter>();
    ...
    public void registerVariables(ArrayList<String> listID, DataValue value, Token token, boolean mode,
            String procedure, int executionNumber) {
    ...
    // Registar el dato
    public void registerVariable(String id, DataValue value, Token token, boolean mode, String procedure,
            int executionNumber) {
        if (this.if_instruc && tablaSimbolos.containsKey(id)) {
            this.repeatedId(id, token);
        } else {
            this.listDebbuge.add(new DataValueDebbuge(value.getValue(), value.getType(), mode, procedure, id,
                    token.getLine(), executionNumber));
    ...
    public DataValue retrieveDataVariableOrFunction(String id, Token token) {
        if (this.if_instruc && tablaSimbolos.containsKey(id)) {
            ...
            if (returnarDataValue != null)
                return returnarDataValue;
    ...
    private void assignNewDataDebbuge(DataValue data, String id, Token token) {
        ...
        if (useData != null) {
            this.listDebbuge.add(new DataValueDebbuge(data.getValue(), data.getType(), useData.isModo(),
                    useData.getProcedure(), id, token.getLine(), useData.getExecutionNumber()));
        }
    }
    ...
    private void errorThereisVariable(String id, Token token) {
        listError.add(new ReportErrorInterpreter(ErrorTypeInTheInterpreter.SEMANTIC, token,
                THAT_VARIABLE_DOES_NOT_EXIST + id));
    }
        ...
        if (operation.getType() == ListTypeData.BOOLEAN) {
            try {
                ...
                this.listError.add(new ReportErrorInterpreter(ErrorTypeInTheInterpreter.SEMANTIC, token,
                        "No es boolean la condiciion de if "));
            }
        } else {
            this.listError.add(new ReportErrorInterpreter(ErrorTypeInTheInterpreter.SEMANTIC, token,
                    "No es boolean la condiciion de if "));
